...
% subplot(1,1,1)
plot(t1,signal);
axis([0 50 -2 2]);xlabel('码元/个');ylabel('幅度');title('源信息序列')
%------------2FSK调制------------
f0 = 5*10^3;
...
figure(2)
subplot(2,1,1);
plot(t,FSK);title('2FSK调制信号时域波形');xlabel('时间/s');ylabel('幅度');
axis([0 0.002 -2 2]);
FSKW = fft(FSK,10*Len);
t1 = 0:Fs/(10*Len-1):Fs;
subplot(2,1,2);
...
volume_jiance = volume_jiance>200;
...
%------------短时傅里叶变换分析跳频信号------------
% figure(5)
% [S W T] = mf_spectrogram(FHMsignal,7000,0,2.5*10^6);
% my_pcolor( W , T , S );

% spectrogram(FHMsignal,7000,1000,1024,2.5*10^6,'yaxis')%处理信号，单次采样点数，相邻两次采样重叠点数，fft点数，采样频率，变换坐标轴
%------------初试条件再设置-------------
% Fs = 2.5*10^6;%采样频率
% Rs = 2.5*10^3;%码元传输速率
% N = 560;%50码元，1码元1000点采样
...
time = 0;
%------------捕获逻辑------------
while (buhuo_ok == 0||queren_ok == 0||tongbu_ok == 0)%进入大循环
    if Send_Clock + Send_STEP  + 3>= MAX_CLOCK%防溢出措施
          fprintf('因数据不够而同步失败!\n')
        break
    end
while (Send_Clock + Send_STEP  + 3<= MAX_CLOCK && buhuo_ok == 0)%进入初始捕获.
...
while(buhuo_ok == 1 && queren_ok == 0)%尝试进入捕获确认
judge2 = judge1;
...
while(queren_ok == 1 && tb < 100 && tongbu_ok == 0)%尝试进入跟踪环节
    if tongbu_end > Len
...
% FHP = [105 245 525 455 385 175 315]*10^3;%Fs/4=625KHz  2FSK第一零点带宽5+2*2.5=10KHz 频谱平移后占25KHz 跳频间隔为70KHz 足够
t2 = 0:1/Fs:(7000-1)/Fs;
...
a1 = fix((tongbu_start+500)/1000);%寻找发射端正确序列
